...
func registerWatchFlags() watchOptions {
	return watchOptions{
		enabled:   flag.Bool("watch", false, "Watch beads data, auto-rebuild, and serve with live refresh"),
		port:      flag.Int("watch-port", 0, "Port for --watch preview server (0 = auto-select 9000-9100)"),
		noBrowser: flag.Bool("no-browser", false, "Do not auto-open browser in --watch mode"),
	}
}

func (o watchOptions) enabledRequested() bool {
	return o.enabled != nil && *o.enabled
}

func (o watchOptions) run(opts exportOptions) error {
	return runWatchMode(opts, *o.port, !*o.noBrowser)
}
